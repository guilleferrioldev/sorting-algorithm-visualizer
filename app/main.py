...
class DrawInformation:
	...
	SIDE_PAD = 100 # Total of pixels as padding on the right and the left side
	TOP_PAD = 150 # Total of pixels as padding on the top

	def __init__(self, width, height, lst):
		self.width = width
		self.height = height

		self.window = pygame.display.set_mode((width, height)) # Create a window
		pygame.display.set_caption("Sorting Algorithm Visualization") # Create a caption for this window
		self.set_list(lst)

	def set_list(self, lst):
		self.lst = lst
		self.min_val = min(lst) # Store the max value of the list
		self.max_val = max(lst) # Store the min value of the list

		self.block_width = round((self.width - self.SIDE_PAD) / len(lst)) # Total of area to represent each of the blocks
		self.block_height = math.floor((self.height - self.TOP_PAD) / (self.max_val - self.min_val))# it determines how high a value should be
		self.start_x = self.SIDE_PAD // 2 # it determines where the block starts from the left to the right


def draw(draw_info, algo_name, ascending):
	draw_info.window.fill(draw_info.BACKGROUND_COLOR)

	title = draw_info.LARGE_FONT.render(f"{algo_name} - {'Ascending' if ascending else 'Descending'}", 1, draw_info.GREEN) # Show the title and render the fonts 
	draw_info.window.blit(title, (draw_info.width/2 - title.get_width()/2 , 5)) # Draw the title over the window

	controls = draw_info.FONT.render("R - Reset | SPACE - Start Sorting | A - Ascending | D - Descending", 1, draw_info.WHITE) # Sow the controls and render the fonts
	draw_info.window.blit(controls, (draw_info.width/2 - controls.get_width()/2 , 45)) # Draw the controls over the window


	sorting = draw_info.FONT.render("I - Insertion Sort | B - Bubble Sort | M - Merge Sort | S - Selection Sort | H - Heap Sort", 1, draw_info.WHITE) # Sow the sorting algorithm and render the fonts
	draw_info.window.blit(sorting, (draw_info.width/2 - sorting.get_width()/2 , 75)) # Show the algorithm over the window

	sorting = draw_info.FONT.render("Q - Quick Sort | X - Radix Sort | L - Shell Sort | O - Cocktail Sort | U - Bucket Sort", 1, draw_info.WHITE)
...
def draw_list(draw_info, color_positions=None, clear_bg=False):
	...
	for i, val in enumerate(lst): 
        # Calculate the x and the y axes from the left corner depending of the width and the height
		...
		pygame.draw.rect(draw_info.window, color, (x, y, draw_info.block_width, draw_info.height)) # Draw in the screen 
...
def radix_sort(draw_info, ascending=True):
	...
	def counting_sort(lst, exp1):
        ...
        # Change count[i] so that count[i] now contains actual position of this digits in putput array
		for i in range(1, 10):
			count[i] += count[i - 1]
    ...
    # Find the maximum number to know number of digits. Do counting sort for every digit.Note that instead of passing digits number, exp is passed expi s 10^i where i is current digit number
	max1 = max(lst)
	exp = 1
	while max1 // exp > 0:
...
# we will use this while loop to render the screen and display the data on screen
	...
	while run:
		clock.tick(120) #max num of time loop runs in 1 sec
